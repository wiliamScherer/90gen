...
# ============================================================================
# 🎨 PALETA DE CORES - CONTROLE CENTRALIZADO
# ============================================================================
# Quer mudar a identidade visual do documento inteiro? É só mexer aqui!

# --- Cores base (ajuste conforme preferência) ---
cor_primaria    <- "#4285F4"
cor_secundaria  <- "#2C5BA9"
cor_destaque    <- "#E63946"
cor_sucesso     <- "#2A9DAF"

# --- Cores derivadas (calculadas a partir das bases) ---
cor_heatmap_low  <- "#f7f7f7"
cor_heatmap_high <- cor_destaque
cor_subtitulo    <- "gray40"

# --- Paleta nomeada (usada nos gráficos via cores["nome"]) ---
cores <- c(
  # Tipos histológicos
  cbc  = cor_primaria,
  cec  = cor_secundaria,
  ...
  # Desfechos binários (sim/não)
  sim  = cor_destaque,
  nao  = cor_sucesso,
  
  # Margens
  livres        = cor_sucesso,
  comprometidas = cor_destaque,
  
  # Visitas
  visita1 = cor_primaria,
  visita2 = cor_secundaria
)
...
theme_set(
  theme_minimal(base_size = 12) +
    theme(
      ...
      plot.subtitle = element_text(color = cor_subtitulo),
...
Hello didi, esse documento é a versão **unificada e turbinada** da análise dos teus dados. Organizei tudo de um jeito que dá pra ver o que está acontecendo, entender os padrões, e — o mais importante — **testar se as diferenças que a gente enxerga são reais ou só obra do acaso**.
...
1.  **Descritiva** — Quem são os pacientes? Como são as lesões?
2.  **Bivariada** — Aqui a gente cruza variáveis buscando associações
3.  **Multivariada** — Aqui vou ser vaidoso: é uma estatística analítica um pouco mais sofisticada
4.  **Recorrência** — Investigação dos pacientes que tiveram duas visitas (qual o problema deles?)
...
#| tbl-cap: "Características dos pacientes"
...
Os artigos costumam apresentar a mediana+IIQ no formato (mediana, IIQ). Sabe qual é o nosso?
Rufem os tambores...
...
**Hipertensão** em `r round(comorb_prev$pct[comorb_prev$comorbidade == "has"])`% da galera. Nenhuma surpresa.
...
CBC representa `r sprintf("%.0f", pct_cbc)`% das lesões. Mas os `r sprintf("%.0f", pct_cec)`% de CEC são mais preocupantes em termos de agressividade.

### Desfechos: o resumo rápido
...
# Parte II: Análise Bivariada {#sec-bivariada}

Agora a gente sai da contagem e entra na **associação**. A pergunta é: *"quem complica mais?"*

::: callout-note
## Por que usamos o Teste Exato de Fisher?

Com poucos eventos (13 complicações, 7 margens comprometidas), o teste de Fisher é mais confiável. Ele calcula a probabilidade exata, sem depender de aproximações que funcionam mal com N pequeno.
:::

## Fatores associados a COMPLICAÇÃO
...
Mas calma — isso pode ser confusão. Lesões que precisam de retalho/enxerto provavelmente são maiores...
:::
...
# Parte III: Análise Multivariada {#sec-multivariada}

A gente ajusta uma variável pela outra pra ver se o efeito **persiste**.

::: callout-warning
## Limitação importante

Com apenas `r n_compl` complicações, não dá pra colocar muitas variáveis no modelo. A regra de bolso é \~10 eventos por variável. Então vou ser conservador: **modelo com 1-2 variáveis**.
:::
...
if (n_compl >= 8) {
  ...
  tbl_mod <- mod_compl |>
    tbl_regression(
      exponentiate = TRUE,
      label = list(fech_simples ~ "Técnica de fechamento", tipo_label ~ "Tipo histológico")
    ) |>
    modify_column_merge(pattern = "{conf.low}, {conf.high}", rows = !is.na(.data$estimate)) |>
    modify_column_hide(columns = "conf.high") |>
    modify_header(label ~ "**Característica**", p.value ~ "**p-valor**", conf.low ~ "**IC 95%**") |>
    modify_footnote(conf.low ~ "IC 95% (inferior, superior)") |>
    modify_source_note("Abreviações: IC = Intervalo de confiança; OR = Razão de chances") |>
    bold_p(t = 0.05) |>
    bold_labels()

  tbl_mod |>
    as_gt() |>
    tab_header(title = "Modelo multivariado: complicação", subtitle = "Regressão logística — OR ajustado (IC 95%)")
} else {
...
Se o OR ajustado continua elevado e significativo, a técnica de fechamento é um preditor **independente** de complicação — não é só porque as lesões eram mais complexas.
:::
...
tbl_visita <- lesoes_visita |>
  ...
  tbl_summary(
    by = visita_label,
    label = list(teve_complicacao ~ "Complicação", margem_comprometida ~ "Margem comprometida", tipo_label ~ "Tipo histológico"),
    statistic = all_categorical() ~ "{n} ({p}%)",
    missing = "no"
  ) |>
  add_p(test = all_categorical() ~ "fisher.test") |>
  modify_header(label ~ "**Característica**", p.value ~ "**p-valor**") |>
  modify_footnote(p.value ~ "Teste exato de Fisher") |>
  bold_p(t = 0.05) |>
  bold_labels()
...
tbl_visita |>
  as_gt() |>
  tab_header(title = "Primeira vs Segunda cirurgia", subtitle = "Os que voltam são diferentes?")
```
...
lesoes_visita |>
  ...
  scale_fill_manual(values = c(cores["visita1"], cores["visita2"]), guide = "none") +
  scale_y_continuous(limits = c(0, 30), labels = \(x) paste0(x, "%")) +
  labs(x = NULL, y = "Taxa de complicação", title = "A segunda cirurgia complica mais?",
       subtitle = "Comparação entre primeira e segunda visita")
```
...
# Parte V: Heatmaps 🔥 {#sec-heatmaps}

Heatmaps são ótimos pra visualizar padrões que tabelas escondem.
...
ggplot(contagens, aes(x = pct_plot, y = histo_subtipo, fill = margem_status)) +
  ...
  scale_fill_manual(values = c("Livres" = cores["livres"], "Comprometidas" = cores["comprometidas"]), name = NULL) +
  labs(x = "Proporção (%)", y = NULL, title = "CBC: margens livres vs comprometidas por subtipo",
...
resumo |> gt() |> tab_header(title = "Resumão", subtitle = "Os números principais")
```
...
3.  **Segunda cirurgia:** Pacientes que voltam merecem atenção.

## Limitações (pra colocar na discussão)

-   **N pequeno:** Poder estatístico limitado.
-   **Dados correlacionados:** Pacientes com múltiplas lesões violam independência.
-   **Viés de seleção:** Quem foi operado? Quem foi pra AP?
...
-   [ ] Revisar os números com o orientador
-   [ ] Decidir quais análises entram no artigo final
-   [ ] Escrever a seção de métodos
-   [ ] Pensar no título do trabalho 😎

------------------------------------------------------------------------

**Uma coisa importante:** tudo que está aqui é pra gente entender os dados. A narrativa final do artigo pode (e deve) ser mais sóbria.

Me conta o que tu achou! 🎯

------------------------------------------------------------------------

*Gerado em `r format(Sys.time(), "%d/%m/%Y às %H:%M")`*
