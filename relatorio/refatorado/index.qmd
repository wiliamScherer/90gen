...
subtitle: '"Nonagenários" me lembra "nona", que me lembra "picão da nona" 💖'
...
if (!file.exists(file.path(renv_root, "90gen.Rproj"))) {
  stop("Não encontrei a raiz do projeto (90gen.Rproj) subindo a partir de: ", proj)
}

# ============================================================================
# 🎨 PALETA DE cor - CONTROLE CENTRALIZADO
# ============================================================================
# Quer mudar a identidade visual do documento inteiro? É só mexer lá!
...
ptbr <- function(x,
  ...
  dict <- list(
    geral = c(
      ...
      "Characteristic" = "Característica", "Overall" = "Total",
      "Abbreviations" = "Abreviações", "Fisher's exact test" = "Teste exato de Fisher",
      "CI = Confidence Interval, OR = Odds Ratio" = "IC = Intervalo de confiança, OR = Razão de chances"
    ),
    header = c("p-value" = "p-valor", "95% CI" = "IC 95%", "Characteristic" = "Característica", "Overall" = "Total"),
    footnote = c("Fisher's exact test" = "Teste exato de Fisher"),
    source_note = c("Abbreviations" = "Abreviações"),
    title = c(), subtitle = c()
  )
...
# ============================================================================
# DADOS ANALÍTICOS PRÉ-PROCESSADOS
# ============================================================================
...
# Cálculos globais usados em várias seções
...
knitr::opts_hooks$set(fig.cap = function(options) {
  ...
  options[["fig.cap"]] <- paste0(
    ...
    "<span style='float:right; font-weight:600;'>",
      "↳ [<a href='", png_file, "' download>plagiar</a>]",
    "</span>"
  )
...
-   **`r nrow(lesoes)` lesões** operadas no total

**Roteiro da nossa jornada espiritual:**

1.  **Estatística descritiva** — Quem são os pacientes? Como são as lesões?
2.  **Análise bivariada** — Aqui a gente cruza variáveis buscando associações
3.  **Análise multivariada** — Aqui vou ser vaidoso: é uma estatística analítica um pouco mais sofisticada
4.  **Recorrência** — Investigação dos pacientes que tiveram duas visitas (afinal, qual o problema deles?)
5.  **Heatmaps** — Meio que um radar pra explorar correlações

------------------------------------------------------------------------

# Estatística Descritiva {#sec-descritiva}

## Quem são esses velhos?
...
tbl_pacientes_gts <- df_tab |>
  ...
  gtsummary::tbl_summary(
    label = list(
      ...
      cbc_cec_previo ~ "Já teve CBC/CEC antes",
      mohs ~ "Fez Mohs",
...
```{r}
#| label: tbl-outros
#| tbl-cap: "Características demográficas dos pacientes"
...
```{r}
#| label: fig-idade
#| fig-cap: "Distribuição de idade da amostra"
...
pacientes |>
  ...
  labs(x = "Idade (anos)", y = "Número de pacientes",
       title = "A idade dos teus pacientes", subtitle = "Todo mundo tem 90+, mas olha a variação!")
...
Olha que interessante: a [mediana](#nota-definicoes) é `r median(pacientes$idade)` anos. Isso significa que metade dos teus pacientes tem **mais** que isso. É uma população bem idosa mesmo — o que faz total sentido pro teu recorte.

::: callout-note
## Por que vamos usar mediana e não média?
...
Só que como dá pra ver no @fig-idade, a nossa amostra é assimétrica (dá pra ver direitinho que a maior massa de pacientes tá à esquerda), e isso distorce a média. Por esse motivo, ela não é a primeira linha no nosso caso.
...
::: text-center
✨**`r paste0("(", sprintf("%.0f", p50), ", ", sprintf("%.0f", p25), "–", sprintf("%.0f", p75), ")")`**✨
:::
...
comorb_prev |>
  ...
  labs(x = NULL, y = "Prevalência", title = "O que eles têm (além do câncer de pele)?",
       subtitle = "Spoiler: hipertensão lidera disparado")
```
...
step <- 0.45  # menor = barras mais próximas (testa 0.45–0.70)
...
carga |>
  ...
  ggplot2::labs(
    ...
    y = "Proporção de pacientes",
    title = "Quantas comorbidades o paciente tem?",
    subtitle = "Entre esses 1,5% tem aqueles pacientes da MI que tem 1000 doenças"
  )
...
#| fig-cap: "Carga de comorbidades por grupos de doenças"

classes_tbl <- tribble(
  ~classe,           ~cols,
  "Cardiovascular",  c("has", "dac", "ic", "fa", "avc", "ivc", "arritmia", "dlp"),
  "Metabólica",      c("dm", "hipotir", "hipertir"),
  "Respiratória",    c("dpoc", "asma"),
  "Neuropsiquiátrica", c("alzheimer", "parkinson", "epilepsia", "depressao", "ansiedade", "esquizofrenia"),
  "Renal",           c("drc"),
  "Oncológica",      c("cbc_cec_previo", "leucemia", "mm", "ca_prostata", "ca_rim", "ca_laringe"),
  "Hábitos",         c("tabagismo", "etilismo", "ex-tabag")
)
...
prev_classe <- classes_tbl2 %>%
  mutate(
    tem = map(cols, ~ {
      # TRUE se o paciente tem >=1 condição na classe
...
prev_classe %>%
  ...
  labs(
    x = NULL,
    y = "Prevalência",
    title = "Comorbidades por grandes classes de doenças",
    subtitle = "(definição de classe: tem pelo menos 1 condição do grupo)"
  )
```
...
## Lesões (do espanhol: Las Lesioñes)
...
lesoes |>
  ...
  labs(x = NULL, y = "Número de lesões", title = "CBC domina, chocando um total de zero pessoas",
...
CBC representa `r sprintf("%.0f", pct_cbc)`% das lesões. O basocelular é o mais comum mesmo (o que é consistente com o que consta na Wikipédia), mas os `r sprintf("%.0f", pct_cec)`% de CEC são mais agressivos (fonte: ChatGPT)
...
A maioria das margens veio livre... isso é bom! Mas tem `r sprintf("%.1f", 100 - pct_livres)`% de margens comprometidas ou parcialmente comprometidas.

Esses casos devem estar ligados diretamente à reintervenção ou acompanhamento mais próximo.

```{r}
#| label: tbl-desfechos-resumo
#| tbl-cap: "Aceite as derrotas - você terá várias"

tibble(
  Desfecho = c("Complicação", "Margem comprometida", "Reintervenção"),
  n = c(n_compl, n_marg, n_reint),
  `%` = sprintf("%.1f%%", 100 * c(n_compl, n_marg, n_reint) / nrow(lesoes))
) |>
  gt() |>
  tab_header(title = "Resumo dos desfechos", subtitle = paste0("N = ", nrow(lesoes), " lesões"))
```
...
#| fig-cap: "Mapa anatômico das lesões"
...
lesoes_local |>
  ...
  labs(
    x = NULL,
    y = "Número de lesões",
    title = "Onde estão as lesões?",
    subtitle = "Face e orelha dominam — faz sentido, são áreas fotoexpostas"
  )
```
...
# Análise Bivariada {#sec-bivariada}

Agora a gente sai das contas de +/- e entra nas contas de * e ÷.

::: callout-note

## Ninguém pergunta mas mesmo assim eu respondo
...
## Fatores associados a *complicações*

```{r}
#| label: tbl-bivariada-complicacao
#| tbl-cap: "Análise bivariada: fatores associados a complicação"

lesoes_biv <- lesoes_full |>
  filter(!is.na(teve_complicacao)) |>
  mutate(
    teve_complicacao_label = if_else(teve_complicacao, "Sim", "Não") |> factor(levels = c("Não", "Sim")),
    sexo_label = factor(sexo, c("f", "m"), c("Feminino", "Masculino")),
    tipo_label = factor(tipo_label, c("CBC", "CEC")),
    fech_simples = case_when(
      fech_label %in% c("Fechamento direto", "Segunda intenção") ~ "Direto/2ª intenção",
      TRUE ~ "Retalho/Enxerto"
    ) |> factor(levels = c("Direto/2ª intenção", "Retalho/Enxerto")),
...
tbl_biv_compl <- lesoes_biv |>
  ...
  tbl_summary(
    ...
    label = list(
      sexo_label ~ "Sexo", idade_cat ~ "Faixa etária", faixa_comorb ~ "Nº comorbidades",
      tipo_label ~ "Tipo histológico", fech_simples ~ "Técnica de fechamento", local_grupo ~ "Localização"
    ),
  ...
  add_overall() |>
  modify_header(label ~ "**Característica**", stat_0 ~ "**Total**", stat_1 ~ "**Não**", stat_2 ~ "**Sim**", p.value ~ "**p-valor**") |>
...
tbl_biv_compl |>
  as_gt() |>
  tab_header(title = "Quem complica mais?", subtitle = "Análise bivariada com teste exato de Fisher")
```
...
-   OR bruto: **`r sprintf("%.1f", or_fech$estimate)`** (IC95%: `r sprintf("%.1f–%.1f", or_fech$conf.int[1], or_fech$conf.int[2])`)
-   p = **`r sprintf("%.3f", or_fech$p.value)`**

Mas calma! Esse resultado aparenta ser bastante frágil.
1.  As lesões não são eventos independentes
    Alguns pacientes têm mais de uma lesão, então as observações não são independentes. Uma prerrogativa do teste de Pescador (Fisher) é a independência das unidades. A dependência normalmente subestima incerteza, e isso pode deixar a análise "otimista" demais...
2.  O resultado tá bem perto (*só a pontinha da goiaba*) do nível de significância
    Resultados com poder estatístico foda costumam ter p < 0.001. Mudanças pequenas em uma célula (1 evento a + ou -) podem cruzar o limiar de 0,05. Em outras palavras, o nosso p ter ficado abaixo de 0,05 pode ser só "sorte".
3.  A definição de complicação pode significar uma infecção leve ou necrose do enxerto
    Pequenas decisões de classificação agregam eventos na categoria "complicação", o que pode distorcer a análise.
4.  A gente fez muitos testes, né?
    Quanto mais testes a gente faz, maior a chance de achar algo "significativo" só por acaso. Isso é o famoso "problema das comparações múltiplas". Quem não conhece?
    Normalmente o jeito de lidar com isso é ajustar o nível de significância (tipo a correção de Bonferroni), que deixa o teste mais "exigente" justamente pra contrabalançar com os vários testes..
5.  Lesões que precisam de retalho/enxerto provavelmente são maiores ou mais complexas
    E isso pode ser o que realmente aumenta o risco de complicação, e não a técnica em si.
6.  Outros fatores (caso os 5 itens anteriores ainda não tenham te convencido)
    Idade, sexo, fragilidade, diabetes, DRC, tabagismo, imunossupressão, localização, tamanho da lesão, fototipo, cirurgia (*a sirurgia ocorreu tudo bem mas seu filho teve uma parada cardiáca*), quem fez a cirurgia (residente x preceptor), tempo de seguimento, essas coisas.
:::
...
#| tbl-cap: "Análise bivariada: fatores associados a margem comprometida"

lesoes_biv_marg <- lesoes_full |>
  filter(!is.na(margem_comprometida)) |>
  mutate(
    margem_label = if_else(margem_comprometida, "Sim", "Não") |> factor(levels = c("Não", "Sim")),
    tipo_label = factor(tipo_label, c("CBC", "CEC")),
    fech_simples = case_when(
      fech_label %in% c("Fechamento direto", "Segunda intenção") ~ "Direto/2ª intenção",
      TRUE ~ "Retalho/Enxerto"
...
tbl_biv_marg <- lesoes_biv_marg |>
  ...
  tbl_summary(
    by = margem_label,
    label = list(tipo_label ~ "Tipo histológico", fech_simples ~ "Técnica de fechamento", local_grupo ~ "Localização"),
    statistic = all_categorical() ~ "{n} ({p}%)",
    digits = all_categorical() ~ c(0, 1),
    missing = "no"
  ) |>
...
tbl_biv_marg |>
  as_gt() |>
  tab_header(title = "Quem tem mais margem comprometida?", subtitle = "Análise bivariada com teste exato de Fisher")
```

------------------------------------------------------------------------

# Análise multivariada {#sec-multivariada}
...
Com só `r n_compl` complicações, não dá pra colocar muitas variáveis no modelo. Uma prática comum, segundo uma postagem que eu vi no Reddit, é \~10 eventos por variável. Então vou ser conservador: **modelo com 1-2 variáveis**.
:::
    ...
    if (n_compl >= 8) {
      ...
      tbl_mod <- mod_compl |>
        gtsummary::tbl_regression(
          exponentiate = TRUE,
          label = list(
            fech_simples ~ "Técnica de fechamento",
            tipo_label ~ "Tipo histológico"
          )
        ) |>
        # Referência: OR=1, IC=NA (vai aparecer como "—")
        gtsummary::modify_table_body(~ .x |>
          dplyr::mutate(
        ...
        # IC com hífen (só onde tem valor)
        ...
        # Notas de rodapé
        gtsummary::modify_footnote(estimate ~ "OR: odds ratio (razão de chances)") |>
        gtsummary::modify_footnote(conf.low ~ "IC 95%: intervalo de confiança de 95% (limite inferior – limite superior)") |>
        gtsummary::modify_footnote(p.value ~ "Teste de Wald; modelo ajustado para técnica de fechamento e tipo histológico") |>
      ...
      tbl_mod |>
        gtsummary::as_gt() |>
        gt::tab_header(
          title = "Modelo multivariado: complicações",
          subtitle = "Regressão logística, resultado em OR ajustado (IC 95%)"
        ) |>
        gt::rm_source_notes()
    
    } else {
      cat("N de eventos muito pequeno para modelo multivariado confiável")
    }
}, silent = TRUE)
if (inherits(res, "try-error")) {
  cat("Erro ao ajustar o modelo multivariado:", conditionMessage(attr(res, "condition")))
}
```
...
-   A segunda cirurgia complica mais?
    Eu faço a pergunta e eu mesmo respondo: não^*^
    ^*^ = os números parecem muito diferentes, mas a análise não é estatisticamente significativas

```{r}
#| label: tbl-visitas
#| tbl-cap: "Comparação entre visita 1 e visita 2"

lesoes_visita <- lesoes_full |>
  mutate(visita_label = factor(visita, c(1, 2), c("1ª cirurgia", "2ª cirurgia")))
...
tbl_visita |>
  ...
  tab_header(
    ...
    subtitle = "Comparação de desfechos entre visitas"
  )
```
...
#| fig-cap: "Taxa de complicação por visita"
...
dados_visita |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = if (interactive()) "Taxa de complicação por visita" else NULL
  ) +
  theme(
...
## Técnica de fechamento × Desfechos

```{r}
#| label: fig-heatmap-fech-desfechos
#| fig-cap: "Heatmap de desfechos por técnica de fechamento"
#| fig-width: 10
#| fig-height: 5

fech_levels <- c("Fechamento direto", "Enxerto", "Retalho", "Segunda intenção")
...
long <- fd |>
  ...
  mutate(desfecho = factor(desfecho,
    ...
    labels = c("Complicação", "Margem comprometida", "Reintervenção")))
...
heat_fd |>
  ...
  labs(x = NULL, y = NULL, fill = "% de lesões com evento",
       title = "Desfechos por técnica de fechamento",
...
resumo <- tibble(
  Indicador = c("Pacientes", "Lesões", "Idade mediana", "Mulheres", "CBC",
                "Taxa de complicação", "Margens comprometidas", "Pacientes com 2ª cirurgia"),
  Valor = c(
...
## La discussión (o que podemos falar sobre os resultados)

### O que parece importar?

1.  **Técnica de fechamento:** retalho e enxerto se associam a mais complicações.
2.  **Subtipos agressivos de CBC:** parecem ter mais margem comprometida, mas N é pequeno pra afirmar.
3.  **Segunda cirurgia:** pacientes que voltam parecem ter mais complicações

### Limitaciónes (pra colocar na discussão)

**Desenho**
1.  Estudo retrospectivo
    Os dados não foram coletados do presente pro futuro, foram coletados do passado pro presente através da análise de prontuário. Isso é bem mais prático, mas vem com o preço de os dados serem coletados com finalidade assistencial, não exsatamente científica
2.  Centro único
    Pegar só uma fatia da população é válido, mas daí a gente tem que entender que os resultados só se aplicam da fatia de onde vieram
3.  Sem grupo controle
    O padrão ouro seria comparar diferentes modalidades de tratamento com um grupo controle,: aí a gente podebater o martelo e dizer "x funciona melhor que y". Aqui a gente só observa o que aconteceu, sem comparar com outra coisa.

**Amostra**
4.  ~Pau~ N pequeno
    Com poucos casos (e principalmente poucos eventos), uma complicação a mais ou a menos muda tudo — inclusive p-valor, OR e IC. O resultado é mais frágil/instável e os intervalos de confiança ficam bem largos.
5.  Viés de sobrevivência
    Olhar só pro passado é seletivo: só entra na amostra quem sobreviveu o suficiente (ou ficou tempo suficiente em seguimento) pra ter o desfecho registrado. Quem sumiu do acompanhamento (por óbito, transferência, fragilidade, perda de retorno) pode estar subrepresentado. Resultado: o estudo pode ser mais otimista com os resultados
6.  Viés de seleção
    Os números dizem respeito a quem chegou e foi operado no serviço, naquele período, com X profissionais, sobrecarga de agenda. Isso também é fonte de viés
    
**Aferição e dados** *(você tem dado em casa?)*
7.  Classificação de complicações
    "Complicação" é um termo muito amplo, e sem critérios rigorosos pode virar um saco de gato
8.  Dados faltantes
...
9.  Tamanho da lesão ausente
    Em termos de cirurgia, tamanho é documento sim! E a gente não tem isso aqui. Talvez porque os registros de prontuário não sejam tão rigorosos nesse ponto.

**Análise estatística**
10. Já comentei: múltiplas comparações
    Quanto mais testes a gente faz, maior a chance de achar algo "significativo" só por acaso. Isso é o *famoso* "problema das comparações múltiplas." *Quem não conhece?*. Mais 
11. Confundimento residual
    Mesmo ajustando por algumas variáveis, sempre vão existir variáveis que a gente não consegue capturar (mesmo em delineamentos super foda/robustos tipo ensaios clínicos randomizados) e que influenciam direta ou indiretamente o resultado.
12. Temporalidade imprecisa
    Análise de prontuário é sempre uma grande merda, né? Tem gente que descreve tipo "semana seguinte", outros "retornou depois", "complicou em algum momento", "abstinência há 6 meses" (3 evoluções no último ano com a mesma hashtag no ctrl+V). Se a data exata não for clara,  fica difícil garantir fatores tipo ordem dos eventos, janela de risco ou comparar seguimentos de forma justa (o número de complicações vai ser diferente se o tempo de seguimento for 3 meses comparado com seguimento de 2 anos, concordas?). Moral da história: quando o desfecho depende do tempo de observação (não sei se é o caso aqui) ele pode embaralhar as interpretações.


------------------------------------------------------------------------

*Feito com muito amor, gerado por R em `r format(Sys.time(), "%d/%m/%Y às %H:%M")`*
